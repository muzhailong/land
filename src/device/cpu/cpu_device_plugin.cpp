...
namespace {
static std::unordered_map<size_t, size_t> mem_usage;
...
void plugin_allocate(const SP_Device* device, uint64_t size,
                     int64_t memory_shape, SP_DeviceMemoryBase* mem) {
    mem->struct_size = SP_DEVICE_MEMORY_BASE_STRUCT_SIZE;
    mem->opaque = aligned_alloc(land::device::cpu::ALIGMENT_BYTE_NUMS, size);
    mem->size = size;
    mem_usage[device->ordinal] = mem_usage.count(device->ordinal)
                                     ? mem_usage[device->ordinal] + size
                                     : size;
}
...
void plugin_deallocate(const SP_Device* device, SP_DeviceMemoryBase* mem) {
    if (mem->opaque != nullptr) {
        free(mem->opaque);
        mem->opaque = nullptr;
        mem_usage[device->ordinal] -= mem->size;
    }
    mem->size = 0;
}

void* plugin_host_memory_allocate(const SP_Device* device, uint64_t size) {
    void* ptr = aligned_alloc(land::device::cpu::ALIGMENT_BYTE_NUMS, size);
    return ptr;
}
...
TF_Bool plugin_device_memory_usage(const SP_Device* device, int64_t* free,
                                   int64_t* total) {
    *total = mem_usage.count(device->ordinal)
                 ? static_cast<int64_t>(mem_usage[device->ordinal])
                 : 0;
    *free = 0;
    return true;
}
...
void plugin_create_stream_executor(const SP_Platform* platform,
    ...
    params->stream_executor->mem_zero =
        [](const SP_Device* device, SP_Stream stream,
           SP_DeviceMemoryBase* location, uint64_t size, TF_Status* status) {};
    params->stream_executor->memset =
        [](const SP_Device* device, SP_Stream stream,
           SP_DeviceMemoryBase* location, uint8_t pattern, uint64_t size,
           TF_Status* status) {};

    params->stream_executor->memset32 =
        [](const SP_Device* device, SP_Stream stream,
...
void SE_InitPluginFns(SE_PlatformRegistrationParams* const params,
    ...
    params->platform->type = PLATFORM_TYPE.c_str();
    params->platform->name = PLATFORM_NAME.c_str();
